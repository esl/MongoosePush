...
We provide already built MongoosePush images. If you just want to use it, then all you need is `docker`, `FCM` app token and/or `APNS` app certificates or token. `APNS` can be configured with either of these - you only need one.
For the full configuration you need to setup the following directory structure:
* priv/
    * ssl/
...
In case you want to use `APNS` token authentication you need to provide token and set `key_id` and `team_id` environmental variables. On how to obtain token and `key_id` read:
...
On how to obtain `team_id` read: https://www.mobiloud.com/help/knowledge-base/ios-app-transfer/
