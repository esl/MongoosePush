defmodule MongoosePush.API.V3FCMTest do
  ...
  test "push to fcm with id mismatch fails" do
    ...
    assert {503, %{"reason" => reason}} = Tools.post(@url, Tools.sample_notification(:fcm))
  end
  ...
  test "push to fcm with unregistered token fails" do
    ...
    assert {410, %{"reason" => reason}} = Tools.post(@url, Tools.sample_notification(:fcm))
  end
  ...
  test "push to fcm with the limit exceeded fails" do
    ...
    assert {429, %{"reason" => reason}} = Tools.post(@url, Tools.sample_notification(:fcm))
  end
